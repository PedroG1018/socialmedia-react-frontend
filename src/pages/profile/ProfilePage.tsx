...
const ProfilePage = () => {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        if (!res.ok)
          throw new Error(data.error || "Couldn't fetch user profile");
  ...
  const { mutate: updateProfile, isPending: isUpdatingProfile } = useMutation({
    mutationFn: async () => {
      try {
        ...
        if (!res.ok)
          throw new Error(
            data.error || "Couldn't change cover/profile pictures"
          );
  ...
  const amIFollowing: boolean = authUser?.following.includes(user?._id);
          ...
          {!isLoading && !isRefetching && user && (
            <>
              ...
              <div className="flex justify-end px-4 mt-5">
                ...
                {(coverPic || profilePic) && (
                  <button
                    className="btn btn-primary rouned-full btn-sm text-white px-4 ml-2"
